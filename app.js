...
window.SEED_DATA = window.SEED_DATA || {
  "schemaVersion": 1,
  "createdAt": "2026-03-18T13:40:19.694779Z",
  "settings": {
    "collectionOrder": [
      "c_tkaniny-2026",
      "c_tkaniny-2025",
  ...
  "collections": {
    ...
    "c_starsze-tkaniny": {
      "id": "c_starsze-tkaniny",
      "name": "STARSZE TKANINY",
      "fabricOrder": [
        "f_aragon",
        "f_aulla",
        "f_barrel",
  ...
  "fabrics": {
    ...
    "f_aragon": {
      "id": "f_aragon",
      "collectionId": "c_starsze-tkaniny",
      "name": "Aragon",
      "colors": {
        "14": "done",
        "13": "todo"
...
(async () => {
  // Kill any old Service Worker + caches that might break updates on GitHub Pages
  try {
...
(() => {
  ...
  const DEFAULT_FIREBASE_CONFIG = {
    ...
    databaseURL: "https://slowtkaniny-default-rtdb.europe-west1.firebasedatabase.app",
    projectId: "slowtkaniny",
    storageBucket: "slowtkaniny.firebasestorage.app",
    messagingSenderId: "384832524818",
    appId: "1:384832524818:web:18a43d5b63805771e7becb",
    measurementId: "G-T7D75MVDKL"
  };
  let isAdmin = false;
  const ADMIN_ACTIONS = new Set([
    "set-color-status","add-colors","delete-color","delete-fabric",
  ...
  const setAdmin = (v, email="") => {
    ...
    if (lb) lb.disabled = isAdmin;
    if (lb2) lb2.disabled = isAdmin;
    if (lo) lo.disabled = !isAdmin;
    if (!isAdmin) {
      bulkMode = false;
  ...
  // ---------- Initialize state ----------
  const seed = window.SEED_DATA || { schemaVersion: 1, createdAt: new Date().toISOString(), settings: { collectionOrder: [], defaultStatus: 'todo' }, collections: {}, fabrics: {} };
  const loadState = () => {
  ...
  const initCloudFromStoredCfg = () => {
    let cfg = loadCloudCfg();
    if (!cfg || !cfg.firebaseConfig) {
      cfg = { firebaseConfig: DEFAULT_FIREBASE_CONFIG, workspaceId: "studio", authMode: "anonymous" };
      saveCloudCfg(cfg);
    }
    if (!cfg.workspaceId) cfg.workspaceId = "studio";
    if (!cfg.authMode) cfg.authMode = "anonymous";
    ...
    try {
      ...
      // Complete redirect sign-in if Google (redirect) was used
      try {
        cloud.auth.getRedirectResult().catch((e) => {
          console.warn(e);
          setCloudStatusText('Błąd logowania: ' + (e.code || 'redirect'));
        });
      } catch {}
      ...
      cloud.auth.onAuthStateChanged(async (user) => {
        ...
        // Auto anonymous sign-in for viewers (no prompt)
        if (!cloud.user) {
          try {
  ...
  const startCloudListener = () => {
    ...
    cloud.unsub = ref.onSnapshot((snap) => {
      if (!snap.exists) {
        // Workspace nie istnieje jeszcze w chmurze — tworzy je tylko administrator.
        if (isAdmin) {
          ref.set({
            schemaVersion: 1,
            ...
            console.warn(err);
            toast('Chmura: brak uprawnień do utworzenia');
          });
        }
        return;
      }
  ...
  const cloudSyncNow = async () => {
    if (!isAdmin || !cloud.user || !cloud.db) { toast('Tylko administrator może synchronizować'); return; }
    try {
...
<body>
  ...
  const explainAuthError = (e) => {
    ...
    if (code === 'auth/operation-not-allowed') {
      return 'W Firebase włącz logowanie Google: Authentication → Sign-in method → Google → Enable.';
    }
    if (code === 'auth/unauthorized-domain') {
      return 'Dodaj domenę w Firebase: Authentication → Settings → Authorized domains → universalwebtools.github.io';
    }
    if (code === 'auth/popup-blocked') {
      return 'Przeglądarka zablokowała okno logowania. Zezwól na wyskakujące okna dla tej strony albo wybierz „Google (przekierowanie)”.';
    }
    if (code === 'auth/popup-closed-by-user') {
      return 'Okno logowania zostało zamknięte. Spróbuj ponownie.';
    }
    if (code === 'auth/cancelled-popup-request') {
      return 'Anulowano poprzednie okno logowania. Spróbuj jeszcze raz.';
    }
    if (code === 'auth/network-request-failed') {
      return 'Błąd sieci (czasem AdBlock/VPN/firewall). Spróbuj bez blokad i odśwież stronę.';
    }
    // default
    return 'Błąd logowania. Otwórz F12 → Console i sprawdź komunikat (e.code).';
  };
  ...
  const cloudLogin = async () => {
    ...
    try {
      ...
      // Redirect mode (more reliable when popups/cookies are blocked)
      if (cloud.authMode === 'google_redirect') {
        await cloud.auth.signInWithRedirect(provider);
        toast('Przekierowanie do logowania…');
        return;
      }
      ...
      setCloudStatusText('Błąd logowania: ' + ((e && e.code) ? e.code : 'nieznany błąd'));
      toast(msg);

      // Fallback: if popup blocked, automatically try redirect once
  ...
  document.addEventListener('click', (e) => {
    ...
    // Top actions
    if (t.id === 'bulkToggleBtn') { if (!isAdmin) { toast('Tylko administrator'); return; } return toggleBulk(); }
    if (t.id === 'undoBtn') { if (!isAdmin) { toast('Tylko administrator'); return; } return undo(); }
    ...
    // Admin buttons
    if (t.id === 'adminLoginBtn' || t.id === 'adminLoginBtn2') return adminLogin();
    if (t.id === 'adminLogoutBtn') return adminLogout();
  ...
  document.addEventListener('change', (e) => {
    ...
    if (t.id === 'importJsonInput' && t.files?.[0]) {
      if (!isAdmin) { toast('Tylko administrator'); t.value=''; return; }
